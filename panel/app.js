...
const nombreCortoProd=s=>{var t=String(s||'');return /pesta|masc/i.test(t)?'Máscara Pestañas':/antena/i.test(t)?'Antena TV':/lente|gafa/i.test(t)?'Lentes One Power':/carga|bater/i.test(t)?'Cargador 12V':/clorofila/i.test(t)?'Clorofila 60 ml':/foco|solar/i.test(t)?'Foco Solar':/ducha|cabezal/i.test(t)?'Cabezal de Ducha':/shilajit/i.test(t)?'Shilajit Ultra':/lymphoria/i.test(t)?'Lymphoria 60 ml':/drainpro|drenaje/i.test(t)?'DRAINPRO':/organiz/i.test(t)?'Organizador Ropa':/almohada|cervical/i.test(t)?'Almohada Cervical':/kinoki|parche/i.test(t)?'Parches Kinoki':/cepillo|parrilla/i.test(t)?'Cepillo Parrilla':t.split('+')[0].trim();};
...
/* ---- veredicto del revisor ----
   El revisor lee la conversacion COMPLETA 15 minutos despues de la venta (incluido
   lo que el cliente dijo despues de registrarla) y deja un nivel. Se pinta aca, al
   lado del boton Aprobar, porque el aviso por WhatsApp casi nunca llega: de 913
   avisos en 7 dias fallaron 872. */
const REV={
  ROJO:    {bg:'#c62828',fg:'#fff',fila:'#fdecec',txt:'#8c1d1d',et:'🔴 NO APROBAR'},
...
/* ---------- Novedades: lo primero que hay que atacar ---------- */
function nvToggle(ix){ var d=document.getElementById("nvDet"+ix); if(d) d.style.display = d.style.display==="none" ? "" : "none"; }
function cargarNovedades(){
  var tb=document.getElementById("tbodyNovedades"); if(!tb) return;
...
/* ---------- ficha completa al hacer clic, molde del CRM de Konecta ---------- */
function _crmDato(x){
  const a=[];
  ...
  p('Motivo',x.motivo);
  p('Abierta hace', x.horas!==undefined?(x.horas+' h'):'');
  p('Días', x.dias!==undefined?x.dias:'');
  p('Mensajes tras recibir', x.mensajes);
  p('Riesgo',x.riesgo);
  p('Bot', x.modo==='agente'?'pausado':'');
  return a.join('');
}
...
async function crmFicha(id){
  ...
  const mover=CRM_COLS.map(function(cc){
    return '<button class="crm-mini'+(crmCol(tar,x)===cc.k?' on':'')+'" onclick="event.stopPropagation();crmMover(\''+id+'\',\''+cc.k+'\');document.getElementById(\'crm-modal\').remove()">'+cc.t+'</button>';
  }).join('');
  m.innerHTML='<div class="crm-panel" onclick="event.stopPropagation()">'+
    ...
    '<div class="crm-sec">Conversación completa</div>'+
    '<div class="crm-chat" id="crmChat"><div class="crm-vacio">Cargando…</div></div>'+
  '</div>';
  ...
  try{
    ...
    box.innerHTML=msgs.map(function(mm){
      const mio=(mm.from!=='cliente');
      const cuerpo=(typeof cuerpoMensaje==='function')?cuerpoMensaje(mm):esc(mm.text||'');
      const hora=mm.time?'<span class="crm-h">'+esc(mm.time)+'</span>':'';
      return '<div class="crm-row'+(mio?' r':'')+'"><span class="crm-b '+(mio?'neg':'cli')+'">'+cuerpo+hora+'</span></div>';
    }).join('');
    box.scrollTop=box.scrollHeight;
  }catch(e){
